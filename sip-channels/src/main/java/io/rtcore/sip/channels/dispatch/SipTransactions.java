...
final class SipTransactions {
  ...
  private static Flowable<SipRequest> transact(final SipRequest req) {
    return Flowable.just(req).repeatWhen(retry -> retry.zipWith(Flowable.range(1, Integer.MAX_VALUE), (n, i) -> i).flatMap(SipTransactions::nextTimer));
  }
