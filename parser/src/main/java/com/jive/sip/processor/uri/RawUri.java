...
public class RawUri implements Uri
{
  ...
  private <T, E extends Uri> T apply(UriVisitor<T> visitor, Parser<E> parser)
  {
    ...
    {
      /*
       * This calls the apply method of the parsed URI rather then simple passing the parsed URI to the visitor so that other URI objects can do other sub-parsing or processing in
       * their own apply methods. One example could be parsing out a URN URI as defined in RFC 5031 (@see com.jive.sip.message.api.uri.UrnUri) that parses out or verfies some some
