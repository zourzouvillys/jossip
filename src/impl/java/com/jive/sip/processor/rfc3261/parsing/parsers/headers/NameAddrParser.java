 ...
 *
 * name-addr is one of the more tricky header values. QuotedString is optional, as is a leading set of tokens and LWS.
 * Then, the URI may or may not be in LAQUOT or RAQUOT.
 *
 */
...
public class NameAddrParser implements Parser<NameAddr>
{
  ...
  // we deviate from RFC 3261 to allow some broken chars in the display-name which a number of devices seem to send.
  private static final String BROKEN_CHARS = "()/";

  public static final Parser<CharSequence> DISPLAY_NAME_CHARS = name(chars(ALPHA_CHARS.concat(DIGIT_CHARS).concat("-.!%*_+`'~").concat(BROKEN_CHARS)),
      "display-name token");
    ...
    try
    {
      ...
      if (name == null)
      {
        ...
        do
        {
          ...
          if ((part == null) || UriParser.RESERVED_CHARS.contains(new String(new byte[]
          { ctx.peek() })))
          {
      ...
      final Collection<RawParameter> rawParams = (ctx.remaining() > 0) ? ctx.read(ParameterParser.getInstance()) : ParameterParser.EMPTY_VALUE;
