...
public class NameAddr extends BaseParameterizedObject<NameAddr> implements Serializable
{
   ...
   * The name.
   * 
   * TODO: we need to keep track of this being a quoted string or not, as a serializaed version should come out the same
   * way it came in.
   * 
   */
  ...
  /**
   * The Uri in this {@link NameAddr}. You may want to use the Uri apply() or adapt() methods to convert it to the type
   * you want.
   */
  @Getter
  ...
  public String toString()
  {
    ...
    if (this.parameters != null)
    {
      for (final RawParameter p : this.parameters.getRawParameters())
      {
        ...
        result += ";" + p.getName() + ((p.getValue() == null) || (p.getValue() instanceof FlagParameterValue) ? "" : "=" + p.getValue());
      }
    }
    return result;
  }
