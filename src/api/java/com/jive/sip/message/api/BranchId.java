...
public class BranchId implements Comparable<BranchId>
{
   ...
   *
   * @param omnomnom The branch value (without the leading magic cookie).
   *
   * @return
   */
  ...
  {
    return (this.getValue().length() > MAGIC_COOKIE.length()) ? this.getValue().substring(MAGIC_COOKIE.length()) : this.getValue();
  }
