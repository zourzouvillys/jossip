...
public class NettyUdpListener implements UdpListener
{
  ...
  public void sendKeepalive(final UdpFlowId flow)
  {
    final DatagramPacket pkt =
        new DatagramPacket(KEEPALIVE.retain(), new InetSocketAddress(flow.getRemote().getHostText(), flow.getRemote()
            .getPort()));
  ...
  public void send(final HostAndPort remote, final SipMessage message)
  {
    ...
    try
    {
      remoteAddress = InetAddresses.forString(remote.getHostText());
    }
    catch (IllegalArgumentException e)
    {
      remoteAddress = InetAddress.getByName(remote.getHostText());
    }
